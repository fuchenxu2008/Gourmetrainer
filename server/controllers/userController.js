...
module.exports = {
    addUser:(req,res)=>{
        const {email,nickname,password,gender} = req.body
        if(!email||!nickname||!password||!gender) return res.send("Please fill in all information")
        User.findOne({email,nickname},(error,existingUser)=>{
    ...
    //Updata
    editUser:(req,res)=>{
        const {email,nickname,password,gender,id} = req.body
        if(!id) return res.status(400).send("User not found")
        User.findByIdAndUpdate(id,{email,nickname,password,gender},{new:true},(error,user)=>{
            if(error) return res.status(400).send(error)
            if(!user) return res.status(404).send("cannot find user")
            return res.json({user,message: "Information changed"})
        })
    },
